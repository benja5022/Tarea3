...
void _pushFrontSortedMap(SortedMap * list, const void * key, const void * value) {
    assert(list != NULL); // list no puede ser NULL.
...
void insertSortedMap(SortedMap * list, const void * key, const void * value){
    ...
    if (!aux || list->compare(aux->key,key)>0) {
        _pushFrontSortedMap (list, key, value);
        return;
    }
...
void * _popFrontSortedMap(SortedMap * list) {
    assert(list != NULL); // list no puede ser NULL.
...
void * _popBackSortedMap(SortedMap * list) {
    assert(list != NULL); // list no puede ser NULL.
...
void * eraseKeySortedMap(SortedMap * list, const void * key) {
    ...
    if (list->current == list->head) {
        return _popFrontSortedMap(list);
    } else if (list->current == list->tail) {
        return _popBackSortedMap(list);
    } else {
        if (aux->next != NULL) {
